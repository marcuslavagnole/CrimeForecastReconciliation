...
## Forecast and calculate residuals for base time series
m_prev = NULL
for(i in 1:tam_teste){
  ...
  prev_aux = matrix(prev,n,(1+(tam_treino+i-1)),byrow=TRUE)
  m_prev   = cbind(m_prev,prev_aux[,1])
  residuo_aux = prev_aux[,2:((tam_treino+i-1)+1)]
  
  # Save residuals
  write.table(residuo_aux, file=paste('residuals/Quarter/residuo',tam_treino+i-1,'.csv',sep=''),sep=';',row.names=FALSE,col.names=FALSE)
  gc()
}         

# Save forecasts
write.table(m_prev,file='base_forecast/base_forecast_quarter.csv',sep=';',row.names=FALSE,col.names=FALSE)